"""Test: POST /api/builds/decode accepts a PoB code and returns BuildData."""
...
# A known-good PoB code (minimal but complete)
...
def test_decode_valid_pob_code():
    """POST /api/builds/decode with a valid PoB code returns 200 + BuildData."""
    response = client.post("/api/builds/decode", json={"pob_code": VALID_POB_CODE})
    ...
    assert data["build"]["level"] == "64"

    # Must have tree specs
    assert "treeSpecs" in data
    assert len(data["treeSpecs"]) >= 1
    assert len(data["treeSpecs"][0]["nodes"]) > 0

    # Must have player stats
    assert "playerStats" in data
    assert "Life" in data["playerStats"]
    assert "TotalDPS" in data["playerStats"]


def test_decode_invalid_code_returns_400():
...
def test_decode_empty_code_returns_422():
    """POST /api/builds/decode with empty string returns 422 (validation)."""
    response = client.post("/api/builds/decode", json={"pob_code": ""})
