...
from app.services.session_context import build_session_context  # S1.0: moved from orchestrator
...
# S1.1: import private helpers from llm_stream (was duplicated inline)
...
36. **装备估价/评价（必须遵守）**：评价或估价一件装备时，**必须先完整分析所有词缀的品质**，再搜市集。
    - **高价值信号（必须逐条识别）**：T1 词缀（如 +3/+4 技能等级、顶级生命/抗性）、破裂词缀（fractured，固定不可改）、稀有附魔（enchant_mods）、Desecrated/Influenced 基底词缀、大师 crafted 词缀、高物品等级（ilvl 82+）。这些在 listings 的 fractured_mods / enchant_mods / crafted_mods / implicit_mods 字段中。
    - **禁止单词缀定价**：一件装备的价值来自**词缀组合**，不是单条最强词缀。+3 召唤项链 + 稀有附魔 + 破裂 + Desecrated 基底 >> 普通 +3 项链。禁止把多条高价值词缀的装备等价于只有一条词缀的地摊货。
    - **搜索对比策略**：估价时 trade_search 的 query 应包含**2-3 条核心词缀**组合搜索（如"+3 召唤等级 项链"搜到的是最便宜单词缀货，不代表用户物品的价值）。要找**类似品质**的在售品做对比，而不是拿最便宜的单属性货对标。
    - **listing_price 的正确解读**：listing_price 是**该次搜索的最低价**，不是用户物品的估价。如果搜索 query 过于宽泛（只含单条词缀），最低价只反映最低档市场，不能代表用户物品的价值。
    - **输出格式**：先列表分析每条词缀的稀有度/价值等级，再综合给出判断。不要跳过词缀直接给结论。
## 回答格式
