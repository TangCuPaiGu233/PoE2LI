...
## 服务器与语言（搜索策略）
- **数据构建**以国际服英文为基准（最完整、最权威），但**服务用户**时以用户的服务器和语言为准。
- 当前用户配置：**国服 + 简体中文**
- 用户用什么语言提问，就用该语言搜索游戏数据。**不要把用户的中文术语翻译成英文再去搜索**。例如用户说"光环"，就搜"光环"，不要转换成"aura"；用户说"野兽"，就搜"野兽"，不要转换成"beast"。
- 如果中文搜索无结果，可补充英文搜索作为 fallback，但**首选始终是用户的语言**。
- 未来系统会让玩家选择所属服务器和语言（如"国际服+英文""国际服+中文"等），届时按用户选择调整搜索目标数据。
"""
